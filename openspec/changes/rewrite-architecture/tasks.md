...
- [x] 4.11 Expose the engine control plane to Python via PyO3 (`submit_batch`, `poll`, `cancel`) — `PyEngine` in `py-bindings/src/engine_mod.rs` exposes `submit_batch(artifact_path, bars_json, spec_json, dataset_manifest) -> handle`, `poll(handle) -> JSON {status, results?, error?}`, `cancel(handle) -> bool`, `drop_handle(handle) -> bool`. Batches run on a `std::thread`-backed worker that loads the strategy cdylib via `StrategyPlugin`, drives `run_batch` + `apply_modes` + `annotate_regimes`, and checks an `AtomicBool` cancel flag between runs. Worker-process implementation (tasks 4.2 / 4.3) is a drop-in swap behind the same surface. New `engine::plugin::PluginFactory` + `OwnedPluginStrategy` make plugin-loaded strategies fit `StrategyFactory`; two integration tests in `engine/tests/plugin_factory.rs` cover the factory + modes path. Python wrapper at `python/strategy_gpt/engine.py` (`Engine`, `JobStatus`); five integration tests in `python/tests/test_engine.py` cover submit/poll/cancel/drop + unknown-handle errors (skipped when native unbuilt).
