...
- [ ] 3.4 Implement the build driver + plugin loader — strategy crates are `cdylib`s that depend on `engine-rt` and implement the sealed `Strategy` trait; a registration macro emitted from `engine-rt` (`strategy_entry!`) generates a C-ABI entry point so the worker can `libloading`-load the compiled `.dylib`/`.so` and obtain a `Box<dyn Strategy>` without recompiling the worker. Build driver runs vanilla `cargo build --release` (with `sccache` if available) over a temp cargo project laid out from the LLM source. — `BuildDriver` orchestration done (lint → cache → cargo via injected `Cargo` trait → store); pending: production `SystemCargo` shellout, the `strategy_entry!` macro on the `engine-rt` side, and the libloading-based loader on the worker side.
