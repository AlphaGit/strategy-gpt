# Spec: progress-reporting

## Purpose

Defines the typed progress-event vocabulary, transport, and rendering contract for long-running Strategy-GPT commands. Establishes a single in-process bus (`ProgressBus`) that orchestrator code and Rust workers publish to, with pluggable sinks (rich TTY live renderer, JSONL, plain text, off) selected per command via `--progress`. Surfaces phase trees, heartbeats, score components, and LLM reasoning previews to humans without polluting stdout or the experiment ledger.
