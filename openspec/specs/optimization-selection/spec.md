...
Overfitting-aware selection layer applied after parameter-optimization search completes and before `best.json` is published. Operates on `trials.parquet` + `manifest.json`; runs as a pure function of those inputs plus selection knobs. Combines a Probability-of-Backtest-Overfitting (PBO) gate via Combinatorially Symmetric Cross-Validation (CSCV), a Deflated Sharpe Ratio (DSR) re-ranking, and an optional parameter-sensitivity (robust) score. Supports post-hoc reselection so methodological knobs (threshold, robust objective, top-K) can be adjusted without re-running the search.
